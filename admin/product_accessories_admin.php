...
<?php include "../templates/menu_header_admin.php" ?>
    ...
    <div style="text-align: center; margin-bottom: 30px;" >
        <a href="add_accessories.php?userId=<?= $userId ?>"><input class="btn btn-primary" type="submit" value="Add product"></a>
    </div>
    <h1 class="products_name">ACCESSORIES</h1>
        <div style="flex-wrap:wrap; width:80%; margin: 0 auto" class="products">
            <?php
            include_once '../config/bd.php';
            $good = addProductsAccessories($connect);
            while ($data = mysqli_fetch_assoc($good)) { ?>
            <a style="margin-top: 10px;" class="card_block" href="edit_product_accessories_card.php?userId=<?= $userId ?>&id=<?= $data[
    'id'
] ?>"><div class="card">
